...
function validateCartQuantity(quantity) {
  ...
  if (!Number.isInteger(quantity)) {
    errors.push('錯誤!數量不是正整數');
  }
  if (quantity < 1 ) {
    errors.push('錯誤!數量小於1了');
  }
  if (quantity > 99) {
    errors.push('錯誤!數量大於99了了');
  }
  const isValid = errors.length === 0;
  return { isValid, errors };
}
...
1. JSON 解析：
fetch：需要手動呼叫 .json() 解析
axios：自動解析 JSON，直接用 response.data

2. 錯誤處理：
fetch：4xx/5xx 不會拋錯，需要自己判斷 response.ok
axios：4xx/5xx 自動拋錯，直接進入 catch

3. 請求設定Content-Type：
fetch：需要手動設定 headers: { 'Content-Type': 'application/json' }
axios：自動設定 Content-Type，資料直接放物件就好
...
const OrderService = {
  ...
  formatOrders(orders) {
    ...
    return orders.map(order => {
      return { ...order,formattedDate:dayjs.unix(order.createdAt).format('YYYY/MM/DD HH:mm')};
      });
  },
